...
def list_packages(chatid, done, status):
    aux = ""
    try:
        cursor = db.rastreiobot.find({"users": str(chatid)}).sort(
            str(chatid), ASCENDING
        )
        qtd = 0
        for elem in cursor:
            if str(chatid) in elem["users"]:
                ...
                if not done:
                    if (
                        "objeto entregue ao" not in status_elem(elem)
                        and "objeto apreendido" not in status_elem(elem)
                        and "objeto roubado" not in status_elem(elem)
                        and "delivered" not in status_elem(elem)
                    ):  # and
                    ...
                    if (
                        "objeto entregue ao" in status_elem(elem)
                        or "objeto apreendido" in status_elem(elem)
                        or "delivered" in status_elem(elem)
                        or "objeto roubado" in status_elem(elem)
                    ):  # or
...
def status_package(code):
    ...
    cursor = db.rastreiobot.find_one({"code": code})
    return cursor["stat"]


# Check if user exists on a specific tracking code
def check_user(code, user):
    cursor = db.rastreiobot.find_one({"code": code.upper(), "users": user})
    if cursor:
        return True
    return False
...
def add_package(code, user):
    ...
    if stat in [status.OFFLINE, status.TYPO]:
        ...
        db.rastreiobot.insert_one(
            {"code": code.upper(), "users": [user], "stat": stat, "time": str(time())}
        )
        stat = status.OK
    return stat
...
def add_user(code, user):
    db.rastreiobot.update_one({"code": code.upper()}, {"$push": {"users": user}})
...
def del_user(chatid, code):
    ...
    db.rastreiobot.update_one({"code": code.upper()}, {"$set": {"users": array}})
...
def set_desc(code, user, desc):
    if not desc:
        desc = code
    db.rastreiobot.update_one({"code": code.upper()}, {"$set": {user: desc}})
...
def cmd_concluidos(message):
    ...
    if len(message) < 1:
        bot.send_message(
            chatid, msgs.not_found, parse_mode="HTML", disable_web_page_preview=True
        )
    else:
...
def cmd_status(message):
    ...
    qtd, wait, despacho, sem_imposto, importado, tributado, trackingmore = (
        count_packages()
    )
    chatid = message.chat.id
    bot.send_message(
        ...
        + "\n"
        + "Taxados em R$15: "
        + str(round(100 * despacho / importado, 2))
        + "%\n"
        +
...
def cmd_statusall(message):
    ...
    bot.send_message(
        ...
        + str(round(100 * despacho / importado, 2))
        + "%\n"
        + "Pacotes sem tributação: "
        + str(round(100 * sem_imposto / importado, 2))
        + "%\n"
        + "Pacotes tributados: "
        + str(round(100 * tributado / importado, 2))
        + "%\n\n"
        + "Mensagens recebidas hoje: "
        + str(todaymsg)
        + "\n"
        + "Mensagens recebidas ontem: "
        + str(yesterdaymsg)
        + "\n\n"
        + "Alertas enviados hoje: "
        + str(today)
        + "\n"
        + "Alertas enviados ontem: "
        + str(yesterday),
        parse_mode="HTML",
    )
...
def cmd_magic(message):
    ...
    if check_type(code) is not None:
        ...
        if exists:
            ...
            if stat == status.OFFLINE:
                ...
                if int(message.chat.id) > 0:
                    ...
                    bot.reply_to(
                        message, "Pacote cadastrado.", reply_markup=markup_clean
                    )
        ...
        if int(user) > 0 and len(message.text) > 25:
            send_clean_msg(
                bot, message.from_user.id, msgs.invalid.format(message.from_user.id)
            )
